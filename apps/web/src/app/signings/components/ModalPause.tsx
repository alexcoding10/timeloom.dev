...
export default function ModalPause() {
  const { openPauseModal, closePauseModal, pauseType,signings,fetchCreatePause } = useSigningsContext()
  const [pauseTypeMap, setPauseTypeMap] = useState<{ value: number, label: string }[]>([])
  const [pauseTypeSelected, setPauseTypeSelected] = useState<{ value: number, label: string } | null>(null)
  ...
  const handlerStartPause = async ()=>{
    ...
    const createPause:CreateTimeBreak = {
      ...
      pauseType:{connect:{id:pauseTypeSelected ? pauseTypeSelected?.value : -1}}, // valor si falla 
  ...
  useEffect(() => {
    ...
    const mapPause = pauseType.map(pause => ({ value: pause.id, label: pause.name }))
      ...
      <Modal.Body className='flex  min-h-[300px] flex-col gap-5 p-5 overflow-visible'>
        ...
        <label>
          ...
          <div className='flex items-center gap-2 bg-zinc-100 p-2 rounded-lg mt-2 text-neutral-dark-400'>
            ...
            <p>
              {
                pauseType && pauseType.find(pause => pause.id === pauseTypeSelected?.value)?.description
              }
            </p>
          </div>
        </label>
        ...
        <label>
          <p className='text-neutral-dark-400'>Puedes escribir una descripcion:</p>
          <textarea
