...
export class CompanyController {
  ...
  async createDeduction(
    @Body() deduction: Prisma.DeductionCreateInput,
    @Res() res: Response,
  ) {
    try {
      ...
      return res
        .status(400)
        .json({ message: 'Error al crear deducción', error });
    }
  }

  @Post('/create/bonus')
  async createBonus(
    @Body() bonus: Prisma.BonusCreateInput,
    @Res() res: Response,
  ) {
    try {
  ...
  async getBalances(
    ...
    return this.companyService.getBalancesCompany(companyId, start, end);
  }
}
